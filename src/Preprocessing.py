...
def preprocess(metrics, recalculate=False, causal=False):
    ...
    if recalculate:
        ...
        try:
            training_data = np.load("../data/COMPAS_train_data.npy")
            training_labels = np.load("../data/COMPAS_train_labels.npy")
            test_data = np.load("../data/COMPAS_test_data.npy")
            test_labels = np.load("../data/COMPAS_test_labels.npy")
...
def metric_vs_recid(metric):
    with open("../data/compas-scores-two-years.csv", "r+") as compas_data:
